...
!      IF( ln_tlu )       CALL tlu_stpdyn    ( kstp, 'Ge' )     ! LU coriolis term
                         CALL dyn_ldf       ( kstp )           ! lateral mixing
!      IF( ln_tlu )       CALL tlu_bcdyn     ( kstp, 'hdf' )    ! LU lateral momentum diffusion
!      IF( ln_tlu )       CALL tlu_bcdyn     ( kstp, 'cdf' )    ! LU mixed lateral-vertical momentum diffusion (TODO)
      IF( ln_zdfosm  )   CALL dyn_osm       ( kstp )           ! OSMOSIS non-local velocity fluxes
                         ...
                         CALL dyn_spg       ( kstp )           ! surface pressure gradient IEEE_DENORMAL FLAGGING(TODO)
...
!      IF( ln_tlu ) THEN
!                            CALL tlu_bcdyn  ( kstp, 'zdf' )    ! LU vertical momentum diffusion
!      ELSE
...
!      IF( ln_tlu )       CALL tlu_tradyn    ( kstp, 'hdf' )   ! LU lateral mixing 
...
!      IF( ln_tlu ) THEN
!                         CALL tlu_tradyn    ( kstp, 'zdf' )   ! LU vertical mixing
!      ELSE
...
!      END IF
      ...
      IF( ln_tlu )       CALL tlu_tvor      ( kstp, 2, un, vn )  ! Vorticity computation: 1 = planetary, 
                                                                 !                        2 = dynamical,
