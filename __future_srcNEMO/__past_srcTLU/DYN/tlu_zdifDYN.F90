MODULE tluzdifDYN
   ...
   PUBLIC   tlu_dyn_zdf   !  routine called by step.F90
...
CONTAINS
   ...
   SUBROUTINE tlu_dyn_zdf( kt )
      ...
      REAL(wp) ::   zWui, zWvi         !   -      -
      REAL(wp) ::   zWus, zWvs         !   -      -
      REAL(wp), DIMENSION(jpi,jpj,jpk)        ::  zwi, zwd, zws, zwA   ! 3D workspace 
      REAL(wp), DIMENSION(:,:,:), ALLOCATABLE ::   ztrdu, ztrdv   !  -      -
      !!---------------------------------------------------------------------
      !
      IF( ln_timing )   CALL timing_start('tlu_dyn_zdf')
      !
      IF( kt == nit000 ) THEN       !* initialization
         IF(lwp) WRITE(numout,*)
         IF(lwp) WRITE(numout,*) 'tlu_dyn_zdf_imp : vertical momentum diffusion implicit operator'
         IF(lwp) WRITE(numout,*) '~~~~~~~~~~~~~~~ '
         !
      ...
      IF ( ln_tlu ) THEN
         !
         ALLOCATE(int_var33(jpi,jpj,jpk) )
         !
         int_var33 = 0._wp
         !
         ...
         DO jk = 1, jpk
            DO jj = 1, jpj
               DO ji = 1, jpi   ! vector opt.
                  zwA(ji,jj,jk) = avm(ji,jj,jk) + int_var33(ji,jj,jk) * 0.5_wp
               END DO
            END DO
         END DO         
         !
      ...
      IF( ln_zad_Aimp ) THEN   !!
            ...
            DO jk = 1, jpkm1
               DO jj = 2, jpjm1 
                  DO ji = fs_2, fs_jpim1   ! vector opt.
                     ...
                     zzwi = - zdt * ( zwA(ji+1,jj,jk  ) + zwA(ji,jj,jk  ) + akzu(ji,jj,jk  ) )   &
                        &         / ( ze3ua * e3uw_n(ji,jj,jk  ) ) * wumask(ji,jj,jk  )
                     zzws = - zdt * ( zwA(ji+1,jj,jk+1) + zwA(ji,jj,jk+1) + akzu(ji,jj,jk+1) )   &
         ...
         CASE DEFAULT               ! iso-level lateral mixing
            DO jk = 1, jpkm1
               DO jj = 2, jpjm1 
                  DO ji = fs_2, fs_jpim1   ! vector opt.
                     ze3ua =  ( 1._wp - r_vvl ) * e3u_n(ji,jj,jk) + r_vvl * e3u_a(ji,jj,jk)   ! after scale factor at U-point
                     zzwi = - zdt * ( zwA(ji+1,jj,jk  ) + zwA(ji,jj,jk  ) ) / ( ze3ua * e3uw_n(ji,jj,jk  ) ) * wumask(ji,jj,jk  )
                     zzws = - zdt * ( zwA(ji+1,jj,jk+1) + zwA(ji,jj,jk+1) ) / ( ze3ua * e3uw_n(ji,jj,jk+1) ) * wumask(ji,jj,jk+1)
                     zWui = ( wi(ji,jj,jk  ) + wi(ji+1,jj,jk  ) ) / ze3ua
                     zWus = ( wi(ji,jj,jk+1) + wi(ji+1,jj,jk+1) ) / ze3ua
                     zwi(ji,jj,jk) = zzwi + zdt * MIN( zWui, 0._wp )
                     zws(ji,jj,jk) = zzws - zdt * MAX( zWus, 0._wp )
         ...
         DO jj = 2, jpjm1     !* Surface boundary conditions
            DO ji = fs_2, fs_jpim1   ! vector opt.
               ...
               zzws = - zdt * ( zwA(ji+1,jj,2) + zwA(ji  ,jj,2) ) / ( ze3ua * e3uw_n(ji,jj,2) ) * wumask(ji,jj,2)
      ...
      IF( ln_zad_Aimp ) THEN   !!
            ...
            DO jk = 1, jpkm1
               DO jj = 2, jpjm1 
                  DO ji = fs_2, fs_jpim1   ! vector opt.
                     ...
                     zzwi = - zdt * ( zwA(ji,jj+1,jk  ) + zwA(ji,jj,jk  ) + akzv(ji,jj,jk  ) )   &
                        &         / ( ze3va * e3vw_n(ji,jj,jk  ) ) * wvmask(ji,jj,jk  )
                     zzws = - zdt * ( zwA(ji,jj+1,jk+1) + zwA(ji,jj,jk+1) + akzv(ji,jj,jk+1) )   &
         ...
         CASE DEFAULT               ! iso-level lateral mixing
            DO jk = 1, jpkm1
               DO jj = 2, jpjm1 
                  DO ji = fs_2, fs_jpim1   ! vector opt.
                     ze3va =  ( 1._wp - r_vvl ) * e3v_n(ji,jj,jk) + r_vvl * e3v_a(ji,jj,jk)   ! after scale factor at V-point
                     zzwi = - zdt * ( zwA(ji,jj+1,jk  ) + zwA(ji,jj,jk  ) ) / ( ze3va * e3vw_n(ji,jj,jk  ) ) * wvmask(ji,jj,jk  )
                     zzws = - zdt * ( zwA(ji,jj+1,jk+1) + zwA(ji,jj,jk+1) ) / ( ze3va * e3vw_n(ji,jj,jk+1) ) * wvmask(ji,jj,jk+1)
                     zWvi = ( wi(ji,jj,jk  ) + wi(ji,jj+1,jk  ) ) / ze3va
                     zWvs = ( wi(ji,jj,jk+1) + wi(ji,jj+1,jk+1) ) / ze3va
                     zwi(ji,jj,jk) = zzwi  + zdt * MIN( zWvi, 0._wp )
                     zws(ji,jj,jk) = zzws  - zdt * MAX( zWvs, 0._wp )
         ...
         DO jj = 2, jpjm1     !* Surface boundary conditions
            DO ji = fs_2, fs_jpim1   ! vector opt.
               ...
               zzws = - zdt * ( zwA(ji,jj+1,2) + zwA(ji,jj,2) ) / ( ze3va * e3vw_n(ji,jj,2) ) * wvmask(ji,jj,2)
      ...
      IF ( ln_tlu ) DEALLOCATE( int_var33 )

      IF( ln_timing )   CALL timing_stop('tlu_dyn_zdf')
      !
   END SUBROUTINE tlu_dyn_zdf

   !!==============================================================================
END MODULE tluzdifDYN
