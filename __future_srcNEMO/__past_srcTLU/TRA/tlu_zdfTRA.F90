MODULE tluzdfTRA
   ...
   PUBLIC   tlu_tra_zdf   ! called by step.F90
   PUBLIC   tlu_zdf_imp   ! called by trczdf.F90
...
CONTAINS
   ...
   SUBROUTINE tlu_tra_zdf( kt, r_Sc )
      !!----------------------------------------------------------------------
      !!                  ***  ROUTINE tra_zdf  ***
      !!
      ...
      REAL(wp), DIMENSION(jpts), INTENT(in   ) ::   r_Sc               ! 1/Schmidt number
      !!---------------------------------------------------------------------
      !
      ...
      IF( kt == nit000 )  THEN
         IF(lwp)WRITE(numout,*)
         IF(lwp)WRITE(numout,*) 'tlu_zdf : TLU implicit vertical mixing on T & S'
         IF(lwp)WRITE(numout,*) '~~~~~~~ '
      ENDIF
      !
      ...
      CALL tlu_zdf_imp( kt, nit000, 'TRA', r2dt, tsb, tsa, jpts, r_Sc ) 
...
!!gm this should be moved in trdtra.F90 and done on all trends
         CALL lbc_lnk_multi( 'tlu_zdf', ztrdt, 'T', 1. , ztrds, 'T', 1. )
!!gm
      ...
      !
      IF( ln_timing )   CALL timing_stop('tlu_zdf')
      !
   END SUBROUTINE tlu_tra_zdf

 
   SUBROUTINE tlu_zdf_imp( kt, kit000, cdtype, p2dt, ptb, pta, kjpt, r_Scale ) 
      !!----------------------------------------------------------------------
      !!                  ***  ROUTINE tra_zdf_imp  ***
      !!
      ...
      REAL(wp), DIMENSION(jpts)            , INTENT(in   ) ::   r_Scale               ! 1/Schmidt number
      !
               ...
               DO jk = 2, jpkm1
                  DO jj = 2, jpjm1
                     DO ji = fs_2, fs_jpim1   ! vector opt.
                        zwt(ji,jj,jk) = zwt(ji,jj,jk) + ( 0.5 * int_var33(ji,jj,jk) / r_Scale(jn) )
                     END DO
                  END DO
               END DO         
            END IF
   ...
   END SUBROUTINE tlu_zdf_imp

   !!==============================================================================
END MODULE tluzdfTRA
