...
# endif
      ...
      IF( kstp == nit000 + dt_delay ) ln_tlu_ = .true.
      IF( ln_tlu_ ) THEN
         IF( ln_tlu_ )        CALL tlu_fields       ( kstp )  ! Compute the noise
                             CALL tlu_wzvcmp
      END IF
...
!!jc: fs simplification
      ...
      IF( ln_tlu_ )       CALL tlu_stpdyn    ( kstp, 'QG' )     ! LU coriolis term
...
!     IF( ln_tlu_ .and. kstp == 3)       CALL noise_energy(  )
