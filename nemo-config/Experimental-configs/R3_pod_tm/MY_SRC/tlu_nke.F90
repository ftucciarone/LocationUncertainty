...
MODULE tlunke
   ...
   INCLUDE 'mpif.h'
   !
   INTEGER                        ::   nsamples                          !< @private number of samples in the ensemble
   ! [tlu_noise]
...
CONTAINS
   ...
   SUBROUTINE tlu_nke_init
      INTEGER(i4) :: ios, ierr   ! namelist output, allocation statistics
      !
      ! Read namelist: transport under location uncertainty parametrization
      !
      NAMELIST/namtlu_nke/   nsamples, &
      &                      amp_noi,  &
      &                      amp_bia      
      !
      ! Read namelist
      !
      REWIND( numnam_ref )
      READ  ( numnam_ref, namtlu_nke, IOSTAT = ios ) ! [TODO] error management - cf module_example
      REWIND( numnam_cfg )
      READ  ( numnam_cfg, namtlu_nke, IOSTAT = ios ) ! [TODO] error management - cf module_example
      !
      !!------------------------------------------------------------------------
      !
      IF (lwp) THEN
         WRITE(numout,*)
         WRITE(numout,*) 'tlu_nke_init : Transport under Location Uncertainty, Noise scaling with energy '
         WRITE(numout,*) '~~~~~~~~~~~~'
         WRITE(numout,*) '   namtlu_nke namtlu'
         WRITE(numout,*) '                   number of samples drawn  nsamples = ', nsamples
         WRITE(numout,*) '                 noise rescaling amplitude   amp_noi = ', amp_noi
         WRITE(numout,*) '                  bias rescaling amplitude   amp_bia = ', amp_bia
      END IF
      !
      !
      ! Allocate
   ...
   SUBROUTINE tlu_nke( kt )
      ...
      DO j = 1, nsamples
         ...
         !
         unke = unke + ( unoi**2 ) * 0.5_wp 
         vnke = vnke + ( vnoi**2 ) * 0.5_wp
         wnke = wnke + ( wnoi**2 ) * 0.5_wp
         !
      END DO
      !
      ...
      wnke = wnke / nsamples
      !
      CALL int3d_ene3c( unke, vnke, wnke, 'Unke', vel_nke, 1._wp )
      !
      amp = amp_noi * SQRT( dom_siz / vel_nke ) ! 0.025
   ...
   SUBROUTINE tlu_amp( kt )
     ...
     IF( kt == nit000 + dt_delay )  THEN
      ...
      IF (ln_tlu_bia) THEN
         !
         CALL int3d_ene3c( ubia_n**2, vbia_n**2, wbia_n**2, 'dom', bia_nke, 1._wp )
         !
        ! call MPI_Comm_rank(MPI_COMM_WORLD, myID, ierr)
        ! IF ( myID == 0 ) THEN
        !    print '(A36, E16.7)', TRIM('       ')//'    final amplitude value:', amp
        ! END IF      
         !
         ubia_n = ubia_n * SQRT( dom_siz / bia_nke ) * amp_bia !0.0005_wp 
         vbia_n = vbia_n * SQRT( dom_siz / bia_nke ) * amp_bia !0.0005_wp
         wbia_n = wbia_n * SQRT( dom_siz / bia_nke ) * amp_bia !0.0005_wp
         !
      END IF
      !
      !
