...
CONTAINS
   ...
   SUBROUTINE tlu_init_dmd
      ...
      IF (ln_tlu_bia) THEN
         ...
         ! Allocate Spatial modes
         !
         ALLOCATE( dmd_rxmd_c(jpi,jpj,nn_tlu_nmod_c * jpk_), &
         ...
         ! Allocate Spatial modes
         !
         ALLOCATE( umean(jpi,jpj,jpk_), &
         &         vmean(jpi,jpj,jpk_), &
         &         wmean(jpi,jpj,jpk_), stat=ierr(4) )
         !
      END IF
      !
   ...
   SUBROUTINE read_vel_mod( )
      ...
      IF (ln_tlu_bia) THEN
         ...
         DO jm = 1,nn_tlu_nmod_c
            ...
            ! Read real parts of Random components
            !
            CALL read3Df4D_REALvar ("read_vel_md", "umode_real_c", ncID, dmd_rxmd_c(:,:,m_idx + 1 : m_idx + jpk_ ), (jm-1)*2 + 1 )
            CALL read3Df4D_REALvar ("read_vel_md", "vmode_real_c", ncID, dmd_rymd_c(:,:,m_idx + 1 : m_idx + jpk_ ), (jm-1)*2 + 1 )
            !
            ! Read imaginary parts of Random component
            !
            CALL read3Df4D_REALvar ("read_vel_md", "umode_imag_c", ncID, dmd_ixmd_c(:,:,m_idx + 1 : m_idx + jpk_ ), (jm-1)*2 + 1 )
            CALL read3Df4D_REALvar ("read_vel_md", "vmode_imag_c", ncID, dmd_iymd_c(:,:,m_idx + 1 : m_idx + jpk_ ), (jm-1)*2 + 1 )
            !
            dmd_rxmd_c(:,:,m_idx + 1 : m_idx + jpk_ ) = dmd_rxmd_c(:,:,m_idx + 1 : m_idx + jpk_ ) * umask * val_c
            dmd_rymd_c(:,:,m_idx + 1 : m_idx + jpk_ ) = dmd_rymd_c(:,:,m_idx + 1 : m_idx + jpk_ ) * vmask * val_c
            dmd_ixmd_c(:,:,m_idx + 1 : m_idx + jpk_ ) = dmd_ixmd_c(:,:,m_idx + 1 : m_idx + jpk_ ) * umask * val_c
            dmd_iymd_c(:,:,m_idx + 1 : m_idx + jpk_ ) = dmd_iymd_c(:,:,m_idx + 1 : m_idx + jpk_ ) * vmask * val_c
            !
         END DO
         !
         ! Read frequency vector
         ...
         !
         umean = umean * umask * 0.01_wp * val_c
         vmean = vmean * vmask * 0.01_wp * val_c
         !
      END IF
      ! 
   ...
   SUBROUTINE tlu_bia_dmd( kt, freq )
      ...
      DO jm = 1, nn_tlu_nmod_c
         ...
         !
         ubia_n(:,:,:) = ubia_n(:,:,:) + 2._wp * dmd_rxmd_c(:,:,m_idx + 1 : m_idx + jpk_ ) * cos( freq(jm) * REAL( kt, wp ) * rn_rdt ) & 
         &                             - 2._wp * dmd_ixmd_c(:,:,m_idx + 1 : m_idx + jpk_ ) * sin( freq(jm) * REAL( kt, wp ) * rn_rdt )
         !
         vbia_n(:,:,:) = vbia_n(:,:,:) + 2._wp * dmd_rymd_c(:,:,m_idx + 1 : m_idx + jpk_ ) * cos( freq(jm) * REAL( kt, wp ) * rn_rdt ) &
         &                             - 2._wp * dmd_iymd_c(:,:,m_idx + 1 : m_idx + jpk_ ) * sin( freq(jm) * REAL( kt, wp ) * rn_rdt )
         !
      ENDDO
      !
   ...
   SUBROUTINE tlu_bia2D_dmd( kt, freq )
      ...
      DO jm = 1, nn_tlu_nmod_c
         ...
         ! Create realization of sigma dBt = \sum_{k} \phi_{k}\xi_{k}
         !
         ubia_(:,:) = ubia_(:,:) + 2._wp * dmd_rxmd_c(:,:, jm ) * cos( freq(jm) * REAL( kt, wp ) * rn_rdt ) & 
         &                       - 2._wp * dmd_ixmd_c(:,:, jm ) * sin( freq(jm) * REAL( kt, wp ) * rn_rdt )
         !
         vbia_(:,:) = vbia_(:,:) + 2._wp * dmd_rymd_c(:,:, jm ) * cos( freq(jm) * REAL( kt, wp ) * rn_rdt ) &
         &                       - 2._wp * dmd_iymd_c(:,:, jm ) * sin( freq(jm) * REAL( kt, wp ) * rn_rdt )
         !
      ENDDO
      !
      ! Mask the noise to enforce No-Flux boundary conditions 
      ...
      DO jk = 1, jpkm1
        !
        ubia_n(:,:,jk) = ubia_ * umask(:,:,jk)
        vbia_n(:,:,jk) = vbia_ * vmask(:,:,jk)
        !
      END DO
      !
