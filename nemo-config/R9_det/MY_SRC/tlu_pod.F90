...
CONTAINS
   ...
   SUBROUTINE read_vel_mod( )
      ...
      DO jm = 1, nn_tlu_nmod
         ...
         !
         !
         pod_xmd(:,:,m_idx + 1 : m_idx + jpk ) = zwa1 * umask * val
         pod_ymd(:,:,m_idx + 1 : m_idx + jpk ) = zwa2 * vmask * val
         pod_zmd(:,:,m_idx + 1 : m_idx + jpk ) = zwa3 * wmask * val
         !
      ENDDO
      !
      ...
      IF (ln_tlu_bia) THEN
         ...
         !
         pod_xav = pod_xav * umask * val
         pod_yav = pod_yav * vmask * val
         !
      END IF
      !
      ! Close netCDF files
