...
CONTAINS
   ...
   SUBROUTINE kern_apply_2fld( self, fldin_u, fldin_v, fldout_u, fldout_v, keep_in_field )
      USE tluMPI
      CLASS(filter),                   INTENT(in   ) :: self
      REAL(wp),      DIMENSION(:,:,:), INTENT(in   ) ::  fldin_u
      REAL(wp),      DIMENSION(:,:,:), INTENT(in   ) ::  fldin_v
      REAL(wp),      DIMENSION(:,:,:), INTENT(inout) :: fldout_u
      REAL(wp),      DIMENSION(:,:,:), INTENT(inout) :: fldout_v
      LOGICAL,       OPTIONAL,         INTENT(in   ) :: keep_in_field
      LOGICAL                                        :: keep_in_field_
      !
      INTEGER                                        :: i, j
      ...
      ! Initialize 
      !
      IF ( .NOT.PRESENT(keep_in_field) ) THEN
         keep_in_field_ = .false.
      ELSE
         keep_in_field_ = keep_in_field
      END IF
      
      IF ( .not.keep_in_field_ ) THEN
         fldout_u = 0._wp
         fldout_v = 0._wp
      END IF
