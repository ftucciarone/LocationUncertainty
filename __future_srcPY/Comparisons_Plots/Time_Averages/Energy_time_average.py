...
filename = ['GYRE_5d_00010101_00021230_grid_',
...
# ------------------------------------------------
HRdata = Dataset(data_dir + sub_dir[0] + HRsubs_dir[0] + "/" +  filename[0] + "U" + ext, 
                      "r", format="NETCDF4")
...
for s in HRsubs_dir:
    ...
    if np.size(HRvardims) == 3:
        fin = Dataset(data_dir + sub_dir[0] + HRsubs_dir[i] + "/" +  filename[0] + grid2plt + ext, 
                      "r", format="NETCDF4")
        HRvar = fin.variables[HRvarname][:, :, :]
        nt += fin.dimensions[HRvardims[0]].size
        HRcmvar += np.sum(HRvar, axis=0)
        del HRvar
    elif np.size(HRvardims) == 4:
        fin = Dataset(data_dir + sub_dir[0] + HRsubs_dir[i] + "/" +  filename[0] + "U" + ext,
                      "r", format="NETCDF4")
        HR_U = fin.variables["vozocrtx"][:, idk, :, :]
        fin.close
        fin = Dataset(data_dir + sub_dir[0] + HRsubs_dir[i] + "/" +  filename[0] + "V" + ext,
                      "r", format="NETCDF4")
        HR_V = fin.variables["vomecrty"][:, idk, :, :]
        HRvar = 0.5*( HR_U**2 + HR_V**2 )
        HRcmvar[:, :] += np.sum(HRvar, axis=0)
        del HRvar
        nt += fin.dimensions[HRvardims[0]].size
    fin.close
...
if np.size(LRvardims) == 3:
    ...
    LRvar = np.mean(LRdata.variables[LRvarname][1:maxTime, idk, :, :], axis=0)
    LUvar = np.mean(LUdata.variables[LRvarname][1:maxTime, idk, :, :], axis=0)

maxval = np.amax(LRvar)/2
print(maxval)
print(np.amax(LUvar))
...
ry[1:] = 1
rx[0] = (nx[0] - 2)/(nx[1] - 2)
ry[0] = (ny[0] - 2)/(nx[1] - 2)      0.360758836739144 0.3673860231827291


fig, (ax1, ax2, ax3) = plt.subplots(1, 3)

im0 = ax1.imshow(np.rot90(HRvar[::-1,:]), cmap='RdBu_r', vmin = 0, vmax = maxval*100000)
im1 = ax2.imshow(np.rot90(LRvar[::-1,:]), cmap='RdBu_r', vmin = 0, vmax = maxval)
im2 = ax3.imshow(np.rot90(LUvar[::-1,:]), cmap='RdBu_r', vmin = 0, vmax = maxval)
...
cb.ax.set_yticklabels([r'0.0', r'-0.1', r'0.2', r'0.3', r'0.4'],
