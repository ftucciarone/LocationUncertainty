...
HRbasefile = ["GYRE_10575360_restart",
              'GYRE_10782720_restart',
              'GYRE_10990080_restart',
              'GYRE_11197440_restart']
HRprnt_lab = [r"R27d"]
grid2plt = 'T'
ext = '.nc'
...
# ------------------------------------------------
HRdata = Dataset(HRbase_dir + HRsubs_dir[0] + "/" +  HRbasefile[0] + ext, 
                      "r", format="NETCDF4")
...
for ids, s in enumerate(HRsubs_dir):
    inputfile = HRbase_dir + s + "/" +  HRbasefile[ids] + ext
...
times = [0, 144, 288, 432, 576]
DTzT = np.empty((5, 30))
LRzT = np.empty((5, 30))
for it, t in enumerate(times):
    if np.size(LRvardims) == 3:
        DTvar = DTdata.variables[LRvarname][t, :, :]
        LRvar = LRdata.variables[LRvarname][t, :, :]
    elif np.size(LRvardims) == 4:
        DTvar = DTdata.variables[LRvarname][t, :-1, :, :]
        LRvar = LRdata.variables[LRvarname][t, :-1, :, :]
...
# Userdef parameters
sup_title = r"Vertical profile of salinity" 
...
for idx, ax in enumerate(axes_list):
    ...
    ax.set_title(r"t=" + str((idx)*2) + "Y")
    ax.set_xlim([33, 36])
    ax.set_xlabel(r"[kg/m$\null^{3}$]")
    


plt.savefig(OUTdir + "Salinity_vertProfile" + ".eps",
