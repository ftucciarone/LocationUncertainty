...
LRprnt_lab = [r"Deterministic", 
              ...
              r"$(                I - \mathcal{F}_{72} )\boldsymbol{u}$",
...
LRbasefile = ['GYRE_5d_00010101_00051230_grid_',
              'euclidean_GYRE_5d_00010101_00051230_grid_',
              'euclidean_GYRE_5d_00010101_00051230_grid_',
              'euclidean_GYRE_5d_00010101_00051230_grid_',
              'euclidean_GYRE_5d_00010101_00051230_grid_',
              'euclidean_GYRE_5d_00010101_00051230_grid_',
...
LU4data = Dataset(LRbase_dir + LRsubs_dir[5] + "/" + LRbasefile[5] +
                  grid2plt + ext, "r", format="NETCDF4")
LU5data = Dataset(LRbase_dir + LRsubs_dir[6] + "/" + LRbasefile[6] +
                  grid2plt + ext, "r", format="NETCDF4")
...
if np.size(LRvardims) == 3:
    ...
    LU5var = LU5data.variables['votemper'][idt, :, :]
...
LU5zT = np.sum(np.multiply(LU5var, LRdAt), axis=( -2, -1)
                )/np.sum(LRdAt, axis=( -2, -1))
...
if np.size(LRvardims) == 3:
    ...
    LU5var = LU5data.variables['vosaline'][idt, :, :]
elif np.size(LRvardims) == 4:
    DTvar = DTdata.variables['vosaline'][idt, :, :, :]
    LRvar = LRdata.variables['vosaline'][idt, :, :, :]
    LU1var = LU1data.variables['vosaline'][idt, :, :, :]
...
LU5zS = np.sum(np.multiply(LU5var, LRdAt), axis=( -2, -1)
               )/np.sum(LRdAt, axis=( -2, -1))
# %% Plotting routine

AxisX = np.empty((2, 7, 30))
# Userdef parameters
sup_title = r"Vertical profile of tracers at instant t=" + str(idt)
sub_title = [r"Temperature", r"Salinity"]

AxisY = depth[:-1]
AxisX[0, :, :] = np.array([DTzT[:-1], LRzT[:-1], LU1zT[:-1], LU2zT[:-1], LU3zT[:-1], LU4zT[:-1], LU5zT[:-1]])
AxisX[1, :, :] = np.array([DTzS[:-1], LRzS[:-1], LU1zS[:-1], LU2zS[:-1], LU3zS[:-1], LU4zS[:-1], LU5zS[:-1]] )
...
for idx, ax in enumerate(axes_list):
    ...
    ax.plot(AxisX[idx, 6, :], AxisY, '--', label=LRprnt_lab[6])
