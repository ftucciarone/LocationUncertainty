...
# ------------------------------------------------
HRdata = Dataset(HRfiles[0]["temp_files"][0], "r", format="NETCDF4")
HRvardims = HRdata.variables["votemper"].dimensions
...
nt = 0
HRvar = np.empty((0,31,542,812))
for i in range(len(HRfiles[0]["temp_files"])):
    print(HRfiles[0]["temp_files"][i])
    if np.size(HRvardims) == 3:
        fin = Dataset(HRfiles[0]["temp_files"][i], "r", format="NETCDF4")
        temp = fin.variables["votemper"][:, :, :]
...
# %%
HRvar_1m = np.mean(HRvar, axis=0)     # First moment
HRvar_2m = np.mean(HRvar**2, axis=0)  # Second moment
...
# -----------------------------------------------
LRdata = Dataset(LRfiles[0]["temp_files"][0], "r", format="NETCDF4")
LUdata = Dataset(LRfiles[1]["temp_files"][0], "r", format="NETCDF4")
...
# -------------------------------------
LRvardims = LRdata.variables["votemper"].dimensions
if np.size(LRvardims) == 3:
    LRvar = LRdata.variables["votemper"][minTime:maxTime, :, :]
    LUvar = LUdata.variables["votemper"][minTime:maxTime, :, :]
...
# %%
title_string = r"$\mathrm{RMS}(T^{\prime}) = " + \
    "\int_{V}(T^{\prime}(x,y,z,t))^{2}\,\mathrm{d}x\mathrm{d}y\mathrm{d}z$"
...
titles = [r"$\mathrm{R27d}$",
          r"$\mathrm{R3d}$",
          r"$\mathrm{R3LU}$"]
title_string = r"$\mathrm{RMS}(T^{\prime})(z) = " + \
    "\int_{A}(T^{\prime}(x,y,z,t))^{2}\,\mathrm{d}x\mathrm{d}y$"
...
# ------------------------------
titles = [r"$\mathrm{RMS}(T_{_{\mathrm{R27d}}}) = " + "{:.2e}".format(HRavg_rms) + "$",
          r"$\mathrm{RMS}(T_{_{\mathrm{R3d}}}) = " + "{:.2e}".format(LRavg_rms) + "$",
          r"$\mathrm{RMS}(T_{_{\mathrm{R3LU}}}) = " +"{:.2e}".format(LUavg_rms) + "$"]
plt.rcParams.update({
    "text.usetex": True,
    "font.family": "serif",
    "font.serif": ["Helvetica"],
