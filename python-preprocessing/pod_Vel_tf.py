...
# 
outfile = "tmfilt_R" + str(high_res) + "toR" + str(3)
...
if not os.path.isfile(outfile + ".nc"): 
    ...
    filt_vels = filt.tm_filt(nt, len(levs), high_res, res, 0.25, fc, dt, [{"u": uvel}, {"v": vvel}, {"w": wvel}])
...
if os.path.isfile(outfile + ".nc"): 
    ...
    utopod = nc.variables["u_hp"][:, levs, 1:-1, 1:-1].data.astype(dtype)
    vtopod = nc.variables["v_hp"][:, levs, 1:-1, 1:-1].data.astype(dtype)
    wtopod = nc.variables["w_hp"][:, levs, 1:-1, 1:-1].data.astype(dtype)
